        ...
        for loader_version in loader_versions:
            ...
            try:
                ...
                if completed.returncode == 0:
                    server_path = root / SERVER_JAR
                    if server_path.exists():
                        return server_path

                    UI.warning(
                        "Fabric genero el launcher, pero no se encontro server.jar; "
                        "descargando el servidor de Minecraft."
                    )
                    self.download("vanilla", version, root)
                    if server_path.exists():
                        return server_path

                installer_output = (completed.stdout or completed.stderr).strip()
                if installer_output:
                    last_error = RuntimeError(installer_output)
...
class ServerManager:
    ...
    def ensure_server_files(self) -> bool:
        ...
        try:
            ...
            required_files = [self.server_jar]
            if server_type == "fabric":
                required_files.append(self.root / FABRIC_LAUNCHER)
            if any(not path.is_file() for path in required_files) or not self.config.install_matches_config():
    ...
    def build_command(self) -> list[str]:
        ...
        server_file = FABRIC_LAUNCHER if server_type == "fabric" else SERVER_JAR
        command = [
            java_path,
            f"-Xms{self.config.get_str('ram_min')}",
            f"-Xmx{self.config.get_str('ram_max')}",
...
def make_start_script(config: Config, windows: bool) -> str:
    ...
    server_file = FABRIC_LAUNCHER if config.get_str("type") == "fabric" else SERVER_JAR
    if windows:
        return (
            "@echo off\n"
            "title MEXUS Minecraft Server\n"
            f'"{java_path}" -Xms{ram_min} -Xmx{ram_max} -XX:+UseG1GC -jar {server_file} nogui\n'
            "pause\n"
        )
    return (
        "#!/usr/bin/env sh\n"
        f'exec "{java_path}" -Xms{ram_min} -Xmx{ram_max} -XX:+UseG1GC -jar {server_file} nogui\n'
    )
